...
def test_greenhouse_maps_every_field():
    jobs = parse_greenhouse("acme-edge", "Acme Edge", samples.GREENHOUSE["acme-edge"])
...
def test_lever_concatenates_description_lists_and_additional():
    ...
    jobs = parse_lever("quantstack", "QuantStack", samples.LEVER["quantstack"])
...
def test_ashby_skips_unlisted_drafts():
    jobs = parse_ashby("helioscale", "Helioscale", samples.ASHBY["helioscale"])
