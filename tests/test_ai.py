...
from findmeajob import ai, backends
from findmeajob.sources import Job
from findmeajob.backends import LLMError, Provider
...
def test_as_list_accepts_array_wrapped_object_and_bare_object():
    assert ai._as_list([{"job_id": "a"}]) == [{"job_id": "a"}]
    assert ai._as_list({"jobs": [{"job_id": "a"}]}) == [{"job_id": "a"}]
    assert ai._as_list({"job_id": "a"}) == [{"job_id": "a"}]
...
def test_screen_splits_into_batches_of_the_configured_size():
    ...
    ai.screen(jobs, PROFILE, batch_size=8, provider=stub, model="m")
...
def test_screen_batch_size_zero_does_not_hang():
    ...
    ai.screen(jobs, PROFILE, batch_size=0, provider=stub, model="m")
    assert len(stub.calls) == 3
...
def test_screen_truncates_the_jd_before_sending():
    ...
    ai.screen(jobs, PROFILE, batch_size=8, jd_chars=1400, provider=stub, model="m")
...
def test_draft_truncates_at_a_larger_limit():
    ...
    ai.draft(jobs, PROFILE, jd_chars=6000, provider=stub, model="m")
...
def test_one_bad_batch_warns_and_the_run_continues(capsys):
    ...
    ai.screen(jobs, PROFILE, batch_size=2, provider=stub, model="m")
...
def test_draft_kit_has_every_key_the_digest_renders():
    ...
    ai.draft(jobs, PROFILE, provider=stub, model="m")

    kit = jobs[0].draft
    assert set(kit) == set(ai.DRAFT_KEYS)
...
def test_failed_draft_leaves_a_renderable_empty_kit(capsys):
    ...
    ai.draft(jobs, PROFILE, provider=stub, model="m")
    assert set(jobs[0].draft) == set(ai.DRAFT_KEYS)
...
def test_stage_provider_overrides_the_global_one(clean_env):
    ...
    screen_p, screen_m = backends.resolve("screen")
    draft_p, draft_m = backends.resolve("draft")
...
def test_explicit_model_wins_over_the_default(clean_env):
    ...
    _, model = backends.resolve("screen")
